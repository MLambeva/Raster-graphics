...
void PGM::load(std::string path)//Функция за прочитане на информация от изображения.
{
	...
	assert(this->ASCIInum == "P2");//Проверка дали е коректен спрямо разширението на файла.
...
void PGM::saveas(std::string path) const//Функция за запазване на информация от изображение във файл, различен от изходния.
{
...
void PGM::print(std::ostream& out) const//Функция за извеждане на информация за даден формат.
{
	out << this->path << '\n';
...
void PGM::rotation(std::string direction) //Завъртане на изображението на 90° в зависимост от зададената посока.
{
...
void PGM::collage(std::string direction, std::string image1, std::string image2, std::string outimage) 
{
	...
	PGM firstImage;//Обект от текущия клас, в който ще съхраняваме данните за първото изображение.
	firstImage.load(image1);

	PGM secondImage;//Обект от текущия клас, в който ще съхраняваме данните за второто изображение.
